...
def get_release_scans(release_file):
    with urllib.request.urlopen(release_file, context=context) as f:
        scan_lines = f.readlines()
        scan_lines = [s.decode("utf-8") for s in scan_lines]
        scans = []
...
def download_task_data(task_data, out_dir):
    ...
    for task_data_id in task_data:
        if task_data_id in TASK_FILES:
            fi = TASK_FILES[task_data_id]
            url = BASE_URL + RELEASE_TASKS + '/' + fi
            localpath = os.path.join(out_dir, fi)
...
def main():
    ...
    parser.add_argument('--id', default='ALL', help='specific scan id to download or ALL to download enture dataset')
