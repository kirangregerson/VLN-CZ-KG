...
class Param:
    def __init__(self):
        ...
        self.parser.add_argument("--finetune", dest="finetune", action="store_const", default=True, const=True)
        self.parser.add_argument("--bert_decay", default=1e-2, type=float)
        self.parser.add_argument("--bert_lr", default=4e-5, type=float)
        self.parser.add_argument("--contrastive", dest="contrastive", action="store_const", default=True, const=True)
        self.parser.add_argument("--con_weight", type=float, default=0.2)
        self.parser.add_argument("--sim", type=str, default='cosine')
        self.parser.add_argument("--load_encoder", type=str, default=None)
        self.parser.add_argument("--load_visual", type=str, default=None)
        self.parser.add_argument("--tune_visual", dest="tune_visual", action="store_const", default=True, const=True)
        self.parser.add_argument("--ndtw", dest="ndtw", action="store_const", default=True, const=True)
        self.parser.add_argument("--visual_before", dest="visual_before", action="store_const", default=False, const=True)
        self.parser.add_argument("--sim_bound", type=int, default=0)
        self.parser.add_argument("--dataset", type=str, default='RxR')
        self.parser.add_argument("--con_lang", type=str, default='multi')
        self.parser.add_argument("--visual_random", dest="visual_random", action="store_const", default=False, const=True)
        self.parser.add_argument("--save_attention", dest="save_attention", action="store_const", default=False, const=True)
        self.parser.add_argument("--eval_environment", dest="eval_environment", action="store_const", default=False, const=True)

        self.parser.add_argument("--visual_combine", dest="visual_combine", action="store_const", default=False, const=True)
        self.parser.add_argument("--objects_constraints", type=str, default=None)
        self.parser.add_argument("--random_objects", dest="random_objects", action="store_const", default=True, const=True)

        # added after ARR Dec review
        self.parser.add_argument("--HAMT", action="store_const", default=False, const=True)
        self.parser.add_argument("--optim_mclip", type=str, default="adam")
        ...
        if self.args.optim_mclip == 'adam':
            print("Encoder Optimizer: Using Adam")
            self.args.optimizer_mclip = torch.optim.Adam
        else:
            print("Encoder Optimizer: BERT AdamW")
