...
class R2RBatch():
            ...
            for item in load_func([split]):
                ...
                if args.dataset == 'RxR':
                    ...
                    if 'instructions' in item:
                        if args.con_lang == 'multi':
                        ...
                        elif args.con_lang == 'en+hi':
                            new_item['multi_instructions'] = []
                            if new_item['language'] == 'te':
                                ...
                                for l, ins in enumerate(item['instructions']):
                                    if item['languages'][l].split('-')[0] != 'te':
                                        new_item['multi_instructions'].append(ins)
                                if len(new_item['multi_instructions']) == 0:
                                    continue
                        elif args.con_lang == 'en+te':
                            new_item['multi_instructions'] = []
                            if new_item['language'] == 'hi':
                                for l, ins in enumerate(item['instructions']):
                                    if item['languages'][l].split('-')[0] == new_item['language']:
                                        new_item['multi_instructions'].append(ins)
                                if len(new_item['multi_instructions']) == 0:
                                ...
                                for l, ins in enumerate(item['instructions']):
                                    if item['languages'][l].split('-')[0] != 'hi':
                                        new_item['multi_instructions'].append(ins)
                                if len(new_item['multi_instructions']) == 0:
                                    continue
                        elif args.con_lang == 'hi+te':
                            new_item['multi_instructions'] = []
                            if new_item['language'] == 'en':
                                ...
                                for l, ins in enumerate(item['instructions']):
                                    if item['languages'][l].split('-')[0] != 'en':
                                        new_item['multi_instructions'].append(ins)
                                if len(new_item['multi_instructions']) == 0:
                                    continue
                        else:
                    ...
                    new_item['elevation'] = item['start_pano']['elevation']
                    new_item['instr_id'] = item['inst_idx']
                    new_item['language'] = 'en-IN'
                    new_item['path_id'] = item['inst_idx']

                    sentence = ''
                    sentence += item['target']
                    for i, dialog in enumerate(item['dialog_history']):
                        sentence += " [SEP] " + dialog["message"] + " "

                    new_item['instructions'] = sentence
                    encoding = self.tok(sentence, padding='max_length', truncation=True, max_length=args.maxInput)
                    new_item['instr_encoding'] = encoding['input_ids']
                    new_item['seq_mask'] = encoding['attention_mask']
                    new_item['seq_length'] = sum(encoding['attention_mask'])
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if args.contrastive and 'multi_instructions' in item:
