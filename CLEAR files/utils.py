...
#import MatterSim #TODO: This will likely be Habitat instead
import string
import json
import time
import math
...
if args.dataset == 'CVDN':
    base_vocab.append('<NAV>')
    base_vocab.append('<ORA>')
    base_vocab.append('<TAR>')
...
def load_datasets_visual(splits):
    """
        :param splits: A list of split.
            if the split is "something@5000", it will use a random 5000 data from the data
        :return:
...
def load_text_features(split, ins_id, language):
    max_length = args.maxInput
    text_features = np.load('../RXR/rxr-data/text_features/rxr_%s/%06d_%s_text_features.npz' % (split, int(ins_id), language))
    encoding = text_features['tokens']
    features = text_features['features']

    if len(features) < max_length:
        seq_length = len(features)
        padding = np.repeat(np.expand_dims(features[-1,:], 0), max_length - len(encoding), 0)
        features = np.concatenate((features, padding), 0)
    elif len(encoding) >= max_length:
        features[max_length - 1, :] = features[-1, :]
        seq_length = max_length

    return encoding, features[:max_length], seq_length
...
def read_object_constraints(file):
    ...
    for scan, v in objects.items():
        new_objects[scan] = dict()
        for view, i in v.items():
            value = None
            for img, name in i.items():
                if value is None:
                    value = np.expand_dims(np.array(name), axis=0)
                else:
                    value = np.concatenate((value, np.expand_dims(np.array(name), axis=0)), axis=0)
            new_objects[scan][view] = value
